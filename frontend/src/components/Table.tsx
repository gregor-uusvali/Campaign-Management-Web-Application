import { Switch, ActionIcon, Input } from "@mantine/core";
...
const TableComponent = ({ campaigns, setCampaigns }: TableComponentProps) => {
  ...
  const renderRows = () => {
    return campaigns.map((campaign) => (
      <Table.Tr key={campaign.id}>
        ...
        <Table.Td>
          <Switch onChange={(e) => toggleIsRunning(campaign.id)} defaultChecked={campaign.is_running} color="green" />
        </Table.Td>
        <Table.Td>
          <ActionIcon onClick={() => deleteCampaign(campaign.id)} color="red" variant="filled" aria-label="Delete">
  ...
  return (
    <><Input  onChange={(e) => setSearchTerm(e.target.value)} m='md'  placeholder="Search" /><Table>
      <Table.Thead>
        <Table.Tr>
